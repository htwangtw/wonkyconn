...
roi_index_single_and_dmn = roi_index_single_network_membership[region_membership.loc[roi_index_single_network_membership, 'yeo7-7']==1]
...
for dk in denoising:
    ...
    if dk == 'baseline':
        ...
        connectomes_prepro = []
        for img, pc in zip(dataset.func, dataset.confounds):
    ...
    average_connectivity_within_network = pd.DataFrame(average_connectivity_within_network, columns=[f'mean_{r}'for r in region_membership.columns[:-1]])
    std_connectivity_within_network = pd.DataFrame(std_connectivity_within_network, columns=[f'sd_{r}'for r in region_membership.columns[:-1]])
